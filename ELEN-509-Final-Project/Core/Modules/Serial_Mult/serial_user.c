...
void ProcessPcPacket(void)
{
  ...
  case 'L':
    ...
    case 'G': {
      ProcessGPS_Packet();
      if (gpsData.gps_lock == true) {
        // Format the response string
        char response[64];
        snprintf(response, sizeof(response), "GPS_LAT:%f;GPS_LONG:%f", gpsData.gps_latitude, gpsData.gps_longitude);
...
{
  // Push the incoming character into the gps_cb buffer
  uint8_t input_char = _buff_instance->buffer[_buff_instance->nextBufferOut];
  gps_cb_push(input_char);
